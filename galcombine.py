...
def main(args):
    ...
    for i in range(1, len(dfse1)):
        df = dfse1[i].copy()
        # df1 = pd.merge(df1, df, how='left', on='des', suffixes=(None, f'{i}'))
        df1 = pd.merge(df1, df, how='outer', on='des', suffixes=(None, f'{i}'))
    for i in range(1, len(dfse2)):
        df = dfse2[i].copy()
        # df2 = pd.merge(df2, df, how='left', on='des', suffixes=(None, f'{i}'))
        df2 = pd.merge(df2, df, how='outer', on='des', suffixes=(None, f'{i}'))
