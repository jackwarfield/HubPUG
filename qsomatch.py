...
d = {'ra': data['ra'].byteswap().newbyteorder(),
...
qso_tot = qso_tot[(qso_tot.R == '-') | (qso_tot.B == '-')].reset_index(drop=True)
...
for fn in fn1:
  ...
  dave = df.d.mean()
  qso = qso_tot
  qso = qso[(np.abs(qso.ra-rave)*np.cos(np.radians(dave)) < 1) & \
      (np.abs(qso.dec-dave)<1)].reset_index(drop=True)
  ...
  for i in qso.index:
    ...
    df['sep'] = np.sqrt((df.r-r)**2 * np.cos(np.radians(d))**2 + (df.d-d)**2)
