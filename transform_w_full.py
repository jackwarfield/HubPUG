...
def main(args):
    ...
    iflag1 = tu.instrument_flag(config.epoch1.instr)
    iflag2 = tu.instrument_flag(config.epoch2.instr)
    ...
    if iflag1:
        fits1_1 = [
            fn.replace(
                ...
                f'{config.epoch1.fitsloc}/{config.epoch1.prefix}',
            )
            for fn in csv1_1
        ]
        fits1_1 = np.array([fn.replace('.csv', '.fits') for fn in fits1_1])
    else:
        fits1_1 = np.array([config.epoch1.fitsloc for fn in csv1_1])
    if iflag2:
        fits2_1 = [
            fn.replace(
                f'{config.epoch2.csvloc}/{config.epoch2.prefix}',
                f'{config.epoch2.fitsloc}/{config.epoch2.prefix}',
            )
            for fn in csv2_1
        ]
        fits2_1 = np.array([fn.replace('.csv', '.fits') for fn in fits2_1])
    else:
        fits2_1 = np.array([config.epoch2.fitsloc for fn in csv2_1])
    ...
    for fn, f1, d1 in zip(csv1, fits1, date1):
        if iflag1:
            ...
            gfn_ = fn.replace(
                f'{config.epoch1.csvloc}/', f'{config.epoch1.gaia}/'
            )
    ...
    print(
        'Sorting tables and calculating Vega magnitudes for the 2nd epoch.\n'
    )
    ...
    for fn, f2, d2 in zip(csv2, fits2, date2):
        if iflag2:
            ...
            # rotate Gaia star lists
            gfn_ = fn.replace(
                f'{config.epoch2.csvloc}/', f'{config.epoch2.gaia}/'
            )
            gfn_ = gfn_.replace('qred.csvsortred.csv', '')
    ...
    gfns = [
        fn.replace(
            f'{config.epoch1.csvloc}/{config.epoch1.prefix}',
            f'{config.epoch1.gaia}/{config.epoch1.prefix}',
        )
        for fn in othercsv
    ]
    gfns = [
        fn.replace(
            f'{config.epoch2.csvloc}/{config.epoch2.prefix}',
            f'{config.epoch2.gaia}/{config.epoch2.prefix}',
        )
        for fn in gfns
    ]
    ...
    for i, (fn, nfn) in enumerate(zip(othercsv, ocsv_match)):
        ...
        ref_m['new_sep'] = (
            (ref_m.X - ref_m.new_X) ** 2 + (ref_m.Y - ref_m.new_Y) ** 2
        ) ** 0.5
    ...
    for i, (fn, nfn) in enumerate(zip(othercsv, ocsv_match)):
        ...
        _ = ref.to_csv(nfn, index=False)
        ref['sep'] = (
            (ref.X - ref.match_X) ** 2 + (ref.Y - ref.match_Y) ** 2
        ) ** 0.5
    ...
    for i, (fn, nfn) in enumerate(zip(othercsv, ocsv_match)):
        ...
        df = pd.read_csv(fn)
        smp_c = smp_cut[
            ['r_X', 'r_Y', f'm_X{i+1}', f'm_Y{i+1}', f'm_sep{i+1}']
        ]
        ...
        smp_cut[f'm_sep{i+1}'] = (
            (smp_cut.r_X - smp_cut[f'm_X{i+1}']) ** 2
            + (smp_cut.r_Y - smp_cut[f'm_Y{i+1}']) ** 2
        ) ** 0.5
        df['last_X'] = df['X']
        df['last_Y'] = df['Y']
        df['X'] = all_new[:, 0]
        ...
        smp_c['new_sep'] = (
            (smp_c.X - smp_c.new_X) ** 2 + (smp_c.Y - smp_c.new_Y) ** 2
        ) ** 0.5
        enddf = smp_c.copy()
    ...
    while loop > 0:
        ...
        for i in range(len(smp_cut)):
            ...
            if (
                (abs(objsep - avesep) > pmtol)
                | (abs(std - avestd) > pmtol)
                | (abs(PMx) > pmtol)
                | (abs(PMy) > pmtol)
            ):
                smp_cut = smp_cut.drop(i, axis=0)
        ...
        for i, (fn, nfn) in enumerate(zip(othercsv, ocsv_match)):
            ...
            smp_c['sep'] = (
                (smp_c.X - smp_c.m_X) ** 2 + (smp_c.Y - smp_c.m_Y) ** 2
            ) ** 0.5
            avesep = smp_c.sep.mean()
            print(f'\t{avesep:.4f} ', end='')
            wgts = np.ones(len(smp_c))
