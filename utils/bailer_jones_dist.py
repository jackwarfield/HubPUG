...
def main(L, plx, plx_err):
    ...
    if type(plx) != type(plx_err):
        print(
            'plx is not the same type as plx_err. Both must either be float or ndarray.'
        )
    ...
    if type(plx) == type(0.0) or type(plx) == np.float64:
        ...
        coeffs = [1.0 / L, -2.0, plx / plx_err**2, -1.0 / plx_err**2]
        roots = np.roots(coeffs)
        real_roots = np.array(
            [
                r
                for x in roots
                for r, i in zip([x.real], [x.imag])
                if np.abs(i) < 1e-15
            ]
        )
        if len(real_roots) == 0:
            print(
                'warning! bailer_jones_dist.py found no valid distance based on the measured parallax of {}, scale length, L = {}, and parallax error of {}. giving NaN instead.'.format(
                    plx, L, plx_err
                )
            )
            return np.nan
        real_pos_roots = real_roots[real_roots > 0.0]
        ...
        print(
            'type of plx and plx_err were not recognized. exiting and returning None. They must be type float or ndarray.'
        )
        return
...
def p(r, L, plx, plx_err):
    ...
    arg = -0.5 / plx_err**2 * (plx - 1.0 / np.exp(r)) ** 2
...
def jac(
    ...
    )  # /(np.exp(r)**3)*f*abs_fac/np.abs(abs_fac)
...
def integrate_w_grid(_func, lim0, lim1, args=[], points=None, limit=None):
    ...
    N_res = 1000.0
    lower = np.log10(1.0 / (plx + 2.0 * plx_err))
    ...
    if plx > 0.0 and 2.0 * plx_err < plx:
        ...
        upper = np.log10(1.0 / (plx - 2.0 * plx_err))
...
def draw(L, _plx, _plx_err, _r_lim, debug=False, u=None):
    ...
    if len(plx_) != len(plx_err_):
        print(
            'length of _plx and _plx_err are not the same, at: {} and {}'.format(
                len(plx_), len(plx_err_)
            )
        )
    ...
    for plx, plx_err in zip(plx_, plx_err_):
        if u is None:
            ...
            _u = u
        if (
            plx_err / plx > 0.1 or plx <= 0.0
        ):   # use the posterior method because the errors are not going to be very symmetric in distance space
            ...
            norm = integrator(
                p, r_lim[0], r_lim[1], args=(L, plx, plx_err), points=points
            )[0]
            if norm == 0.0:
                print(
                    'could return a distance for plx {:4.3e} {:4.3e} because of underflow in the integral'.format(
                        plx, plx_err
                    )
                )
                print(
                    'these cases have found to happen for negative plx and very small plx_err, in which case the exp(plx - 1/r) makes non-infinite r so unlikely as to underflow the calculation of the integral over the posterior probability. returning NaN.'
                )
            ...
            jac_function = functools.partial(
                jac,
                u=_u,
                L=L,
                plx=plx,
            ...
            r_b = minimize_scalar(
                function, bounds=(r_lim[0], r_lim[1]), method='Bounded'
            ).x
            # JCZ 130418
            ...
            lower = np.log10(1.0 / (plx + 4.0 * plx_err))
            if 4.0 * plx_err >= plx:
                upper = lower + 0.5
            else:
                upper = np.log10(1.0 / (plx - 4.0 * plx_err))
            ...
            result.append(
                normal.ppf(_u, loc=1.0 / plx, scale=plx_err / plx**2)
            )
...
def get_err(L, _plx, _plx_err, _r_lim, debug=False):
    ...
    u_upper = 1.0 - (1.0 - sigma_to_percent(1.0)) / 2.0
